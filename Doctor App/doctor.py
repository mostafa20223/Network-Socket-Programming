...
ui,_ = loadUiType('doctor_design.ui')
...
class login(QWidget, login):
    ...
    def Handel_login(self):
        self.db = mysql.connector.connect(host = 'localhost', user = 'root', password = 'DARSH1999', db = 'laboratory')
...
class MainApp(QMainWindow, ui):
    def __init__(self):
        ...
        self.info_tab.setIcon(icon)
        icon0 = QIcon()
        icon0.addPixmap(QPixmap("icons/patients.jpg"), QIcon.Normal, QIcon.Off)
        self.patient_tab.setIcon(icon0)

    def Handel_buttons(self):
        self.tabWidget.tabBar().setVisible(False)
        self.patient_tab.clicked.connect(self.open_patient_tab)
        self.info_tab.clicked.connect(self.open_info_tab)
    ...
    def show_laboratory(self):
        ...
        data = self.cur.fetchall()
        if data:
            self.laboratory_table.setRowCount(0)
            self.laboratory_table.insertRow(0)
            for row, form in enumerate(data):
                for column, item in enumerate(form):
                    self.laboratory_table.setItem(row, column, QTableWidgetItem(str(item)))
                    column += 1
                row_position = self.laboratory_table.rowCount()
                self.laboratory_table.insertRow(row_position)    
